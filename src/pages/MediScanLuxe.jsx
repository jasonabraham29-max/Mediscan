...
export default function MediScanLuxe() {
  ...
  const reset = () => { setStep(1); setDescription(""); setSelected([]); setAge(""); setResult(null); setError(null); };
    ...
    <div style={{ minHeight: "100vh", background: "#F8FBFF" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        :root { --navy:#0A1628; --blue:#1B6FBF; --sky:#5BA8E5; --ice:#EBF4FF; --gold:#C9A84C; --border:rgba(91,168,229,0.15); }
        @keyframes rise { from{opacity:0;transform:translateY(24px)} to{opacity:1;transform:translateY(0)} }
        @keyframes fadeIn { from{opacity:0} to{opacity:1} }
        @keyframes spin { to{transform:rotate(360deg)} }
        .rise { animation: rise 0.7s cubic-bezier(0.16,1,0.3,1) forwards; opacity:0; }
        .d1{animation-delay:0.05s} .d2{animation-delay:0.15s} .d3{animation-delay:0.25s} .d4{animation-delay:0.38s} .d5{animation-delay:0.5s}
        .chip { display:inline-flex; align-items:center; gap:6px; padding:9px 16px; border-radius:50px; border:1px solid var(--border); background:white; color:var(--blue); font-family:'DM Sans',sans-serif; font-size:13px; font-weight:500; cursor:pointer; transition:all 0.2s; }
        ...
        .cta { display:inline-flex; align-items:center; gap:10px; padding:16px 32px; border-radius:50px; border:none; background:var(--navy); color:white; font-family:'DM Sans',sans-serif; font-size:15px; font-weight:500; cursor:pointer; transition:all 0.3s; }
        ...
        .ghost { display:inline-flex; align-items:center; gap:8px; padding:15px 24px; border-radius:50px; border:1.5px solid var(--border); background:transparent; color:var(--blue); font-family:'DM Sans',sans-serif; font-size:15px; font-weight:500; cursor:pointer; transition:all 0.2s; }
        .ghost:hover { border-color:var(--sky); background:var(--ice); }
        .field { width:100%; padding:16px 20px; border-radius:14px; border:1px solid var(--border); background:white; font-family:'DM Sans',sans-serif; font-size:15px; color:var(--navy); outline:none; transition:border-color 0.2s,box-shadow 0.2s; resize:none; line-height:1.6; }
        .field:focus { border-color:var(--sky); box-shadow:0 0 0 4px rgba(91,168,229,0.1); }
        .field::placeholder { color:#B8C5D3; }
        .card { background:white; border-radius:20px; border:1px solid var(--border); box-shadow:0 2px 32px rgba(27,111,191,0.06); padding:28px; }
        .gold-line { height:2px; background:linear-gradient(90deg,transparent,var(--gold),transparent); border:none; }
        .snode { width:30px; height:30px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-family:'DM Sans',sans-serif; font-size:12px; font-weight:600; flex-shrink:0; transition:all 0.3s; }
        ...
        <div style={{position:"fixed",inset:0,zIndex:100,background:"rgba(8,16,30,0.8)",backdropFilter:"blur(16px)",display:"flex",alignItems:"center",justifyContent:"center",padding:24,animation:"fadeIn 0.4s ease"}}>
          <div style={{background:"white",borderRadius:24,maxWidth:440,width:"100%",overflow:"hidden",boxShadow:"0 40px 80px rgba(10,22,40,0.3)"}}>
            ...
            <div style={{padding:"32px 32px 28px"}}>
              ...
              <p style={{fontFamily:"'DM Sans',sans-serif",color:"#6B7A8D",fontSize:14,lineHeight:1.65,marginBottom:18}}>MediScan est un <strong style={{color:"#1B6FBF"}}>outil d'information médicale</strong>, pas un substitut à une consultation médicale.</p>
      ...
      {step === 2 && (
        <main style={{maxWidth:720,margin:"0 auto",padding:"56px 28px 72px"}}>
          ...
          <div className="card" style={{marginBottom:14}}>
            ...
            {selected.length > 0 && (
              <div style={{marginBottom:20}}>
                <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,fontWeight:600,color:"#93A5B8",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:8}}>Symptômes</div>
      ...
      {step === 3 && result && (
        <main style={{maxWidth:720,margin:"0 auto",padding:"56px 28px 72px",animation:"fadeIn 0.6s ease"}}>
          <div style={{background:"linear-gradient(135deg,#0A1628,#1B3A6B)",borderRadius:20,padding:"32px 36px",marginBottom:20,position:"relative",overflow:"hidden"}}>
            ...
            <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(201,168,76,0.15)",borderRadius:50,padding:"5px 14px",marginBottom:14}}>
