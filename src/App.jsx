        ...
        <div style={{maxWidth:1100,margin:"0 auto",padding:"0 28px",height:64,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          {user ? (
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              ...
              <span style={{fontFamily:"'DM Sans',sans-serif",fontSize:13,color:"#6B7A8D",maxWidth:140,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                {user.email}
              </span>
        ...
        <main style={{maxWidth:720,margin:"0 auto",padding:"56px 28px 72px",animation:"fadeIn 0.6s ease"}}>
          ...
          {result.signes_alarme?.length > 0 && (
            <div className="result-section" style={{background:"#FFF5F5",border:"1px solid rgba(192,57,43,0.2)"}}>
              <div className="result-label" style={{color:"#C0392B"}}>
                ⚠️ Appelez le 15 si vous ressentez
              </div>
              <ul className="result-list">
                {result.signes_alarme.map((s,i) => (
                  <li key={i} style={{color:"#7B1A1A"}}>
                    <span style={{color:"#C0392B",fontWeight:600,flexShrink:0}}>—</span>{s}
                  </li>
                ))}
              </ul>
            </div>
